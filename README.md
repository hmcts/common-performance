...
- [IDAM Helper](#-idam-helper)
  - [Create Citizen](#-create-citizen)
...
A helper utility for creating and deleting IDAM citizen users during performance tests.

---

## 🧰 Features
- Create citizen users with generated or predefined credentials
- Store credentials in configurable Gatling session fields
- Create multiple users within the same scenario without overwriting session values
- Delete users using their Gatling session field
...
Creates an IDAM citizen user and stores the email address and password in the Gatling session.

```scala
IdamHelper.CreateCitizen(
  email,
  password,
  emailFieldName,
  passwordFieldName
...
**Parameters:**
- `email` – optional email address override; a unique email is generated by default
- `password` – optional password override; defaults to `Pa55word11`
- `emailFieldName` – Gatling session field used to store the email; defaults to `emailAddress`
- `passwordFieldName` – Gatling session field used to store the password; defaults to `password`

**Example:**

```scala
.exec(IdamHelper.CreateCitizen())
```

This creates a user and stores the credentials as:
...
**Example with custom session field names:**

```scala
.exec(IdamHelper.CreateCitizen(
  emailFieldName = "applicantEmail",
  passwordFieldName = "applicantPassword"
))
```

The credentials can then be referenced using:
...
**Example with predefined credentials:**

```scala
.exec(IdamHelper.CreateCitizen(
  email = "citizen@example.com",
  password = "Pa55word11",
  emailFieldName = "citizenEmail",
  passwordFieldName = "citizenPassword"
))
```

**Outputs:**
- The created user's email address in the configured email session field
- The created user's password in the configured password session field

> 📣 **Note:** Custom session field names should be used when creating multiple citizens in the same scenario to prevent later users from overwriting earlier credentials.

### ➖ Delete Citizen

Deletes an IDAM citizen using an email address stored in the Gatling session.

```scala
IdamHelper.DeleteCitizen(emailFieldName)
```

**Parameters:**
- `emailFieldName` – Gatling session field containing the user's email address; defaults to `emailAddress`

**Example:**

```scala
.exec(IdamHelper.DeleteCitizen())
```

**Example with a custom session field:**
...
The delete request is only performed if the configured email session field exists.
